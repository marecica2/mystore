...
public class ResourceConfig extends ResourceServerConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		http
				...
				.csrf().disable().anonymous().disable().authorizeRequests().antMatchers(HttpMethod.OPTIONS).permitAll()
				// when restricting access to 'Roles' you must remove the "ROLE_" part role
				// for "ROLE_USER" use only "USER"
				.antMatchers("/api/users").access("hasAnyRole('ADMIN')").antMatchers("/api/hello")
				.access("hasAnyRole('USER')").antMatchers("/api/me").hasAnyRole("USER", "ADMIN")
